      ...
      if (payloadSizeMB > MAX_PAYLOAD_MB) {
        setError({
          ...
          details: wasFlattened
            ? `Flattening was applied but the bundle still has too many resources.`
            : `The bundle contains ${currentBundle.entry?.length || 0} resources.`
        })
        setIsGenerating(false)
        return
      }
        ...
        try {
          ...
          if (response.ok) {
            ...
            content = JSON.stringify({
              ...
              section: [{ title: 'Summary', text: { status: 'generated', div: `<div xmlns="http://www.w3.org/1999/xhtml">${summaryResult.summary.replace(/\n/g, '<br/>')}</div>` } }]
            }, null, 2)
          }
        } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">FHIR Summarizer</h1>
                <p className="text-sm text-gray-600">AI-powered health data summarization</p>
              </div>
            </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-8">
            ...
            {error && dataSourceTab === 'upload' && (
              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
                <div className="text-yellow-800 text-sm">
                  Server connection failed. Using file upload as fallback.
                </div>
              </div>
            )}
          </div>
                    ...
                    <div className="flex-1">
                      ...
                      {error.retryable && (
                        <button
                          onClick={generateSummary}
                          disabled={isGenerating}
                          className="mt-3 inline-flex items-center px-3 py-2 border border-yellow-300 text-sm font-medium rounded-md text-yellow-700 bg-white hover:bg-yellow-50 disabled:opacity-50"
                        >
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="border-t border-gray-200 pt-4">
                      ...
                      <div className="flex gap-2">
                        <button onClick={() => handleDownload('txt')} className="px-3 py-2 text-sm bg-gray-100 text-gray-700 rounded hover:bg-gray-200">
                          Download TXT
                        </button>
                        <button onClick={() => handleDownload('json')} className="px-3 py-2 text-sm bg-gray-100 text-gray-700 rounded hover:bg-gray-200">
                          Download JSON
                        </button>
                        <button onClick={() => handleDownload('fhir')} className="px-3 py-2 text-sm bg-gray-100 text-gray-700 rounded hover:bg-gray-200">
                          Download FHIR
                        </button>
                      </div>
                    </div>
          ...
          <footer className="mt-12 text-center text-sm text-gray-500">
            ...
            <p className="mt-1">For demo purposes only. Do not use with real patient data.</p>
          </footer>
        </div>
      </main>
    </div>
